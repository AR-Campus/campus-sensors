...
func TestPasedSensorData(t *testing.T) {
	et := expectations.NewT(t)
	result := sensors.ConvertInfos(rawFireFlyPackage())
	fmt.Println("Test entrys of parsed_Map of Package 6:", result[6].SensorValues)
...
func TestPasedSingleSensorData(t *testing.T) {
	et := expectations.NewT(t)
	result := sensors.ConvertSingle(singelFireFlyData())
	fmt.Println("Test entry time:", result[0].Time)
	// fmt.Println("Input String length", len(rawSensorDataPackets))
	et.ExpectThat(len(result)).DoesNotEqual(0)
